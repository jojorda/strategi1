...
class FokusInputController extends GetxController
  ...
  FokusInputController(this._modaRepository, this._focusRepository,
  ...
  void onInit() {
    ...
    getCctvData(isRefresh: true);
    getSimpulData(isRefresh: true);
      ...
      _strategiRepository.getDataPantau().listen(
        (result) {
          if (result != null) {
            ...
            print('--------------------------------');
            print(
                'ini value simpul jalan : ${simpulData.value?.jalan?.length}');
            print('ini value simpul laut : ${simpulData.value?.laut?.length}');
            debugPrint(
                'ini value simpul udara : ${simpulData.value?.udara?.length}');
            debugPrint('ini value simpul ka : ${simpulData.value?.ka?.length}');
            debugPrint(
                'ini value simpul toll : ${simpulData.value?.toll?.length}');
            debugPrint(
                'ini value simpul toll2 : ${simpulData.value?.toll2?.length}');
            debugPrint(
                'ini value simpul stasiun : ${simpulData.value?.stasiun?.length}');
            debugPrint('--------------------------------');
          }
        },
        onDone: () {
  ...
  void updateFokusData() {
    titleController.text = focusData.value?.judul ?? '';
    simpulTransportasiInput = simpulTransportasiInput
      ..addAll(focusData.value?.simpulTransportasi
              ?.whereType<SimpulTransportasi>() ??
          []);

    cctvInput = cctvInput
      ..addAll(focusData.value?.cctv?.whereType<CctvData>() ?? []);

    video30sInput.addAll(focusData.value?.video30s
            ?.map((video) => FocusVideo((b) => b
              ..judul = video.judul
              ..link = video.link))
            .toList() ??
        []);

    videoLiveStreamingInput.addAll(focusData.value?.videoLiveStreaming
            ?.map((liveStream) => FocusVideo((b) => b
              ..judul = liveStream.judul
