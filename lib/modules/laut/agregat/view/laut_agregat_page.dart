...
class LautAgregat extends GetView<LautController> {
  static const routeName = '/aggregate';

  const LautAgregat({super.key});
  ...
  Widget _buildNavigationArrows() {
    return Obx(() {
      ...
      return _buildArrow(
        ...
        onTap: () {
          controller.showMerakData.value = !controller.showMerakData.value;
  ...
  Widget _buildLegendItem({
    ...
    return Row(
      children: [
        ...
        Text(
          value,
          style: Get.textTheme.labelMedium!.copyWith(color: color, fontSize: isUnfolded ? 14 : (isSmallScreen ? 10 : 11)),
        ),
        const SizedBox(width: 2),
        Text(label, style: Get.textTheme.labelMedium!.copyWith(fontSize: isUnfolded ? 14 : (isSmallScreen ? 10 : 11))),
      ],
    );
  }
  ...
  Widget _buildSailingSection(BuildContext context) {
    final screenHeight = MediaQuery.of(context).size.height;
    final screenWidth = MediaQuery.of(context).size.width;
    final isSmallScreen = screenHeight < 700 || screenWidth < 380;
    final isUnfolded = screenWidth > 600;
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  'Penumpang Pelayaran Domestik & International',
                  style: Get.textTheme.titleSmall!.copyWith(fontSize: isUnfolded ? 14 : (isSmallScreen ? 14 : 14)),
                ),
              )
            ],
          ),
          _buildSailingChart(type: 'passengers'),
          ...
          Row(
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  'Pelayaran Domestik & International',
                  style: Get.textTheme.titleSmall!.copyWith(fontSize: isUnfolded ? 14 : (isSmallScreen ? 14 : 14)),
                ),
              )
            ],
          ),
          _buildSailingChart(type: 'ship'),
          const SizedBox(height: 8),
        ],
      ),
    );
  ...
  Widget _buildSailingChart({required String type}) {
    assert(type == 'passengers' || type == 'ship');

    return Obx(() {
      final chartData = type == 'passengers'
          ? controller.currentAggregateData.value?.chartSailing?.passengers
          : controller.currentAggregateData.value?.chartSailing?.ship;
      ...
      final maxValue = chartData.values.fold(1, max);

      return SizedBox(
        height: 100,
        child: _buildSailingCartesianChart(chartData, maxValue.toDouble()),
      );
    });
  }

  Widget _buildSailingCartesianChart(
      Map<String, dynamic> chartData, double maxValue) {
    return SfCartesianChart(
      ...
      series: <CartesianSeries<MapEntry<String, int>, String>>[
        StackedBarSeries<MapEntry<String, int>, String>(
          dataSource: _mapSailingData(chartData),
          xValueMapper: (data, _) => _mapSailingToLabel(data),
  ...
  String _mapSailingToLabel(MapEntry<String, dynamic> data) {
    switch (data.key) {
      case 'inbound':
        return 'Inbound-International';
      case 'outbound':
        return 'Outbound-International';
      case 'domestic':
        return 'Domestic';
      default:
        return data.key;
  ...
  List<MapEntry<String, int>> _mapSailingData(Map<String, dynamic> chartData) {
    return List<MapEntry<String, int>>.from(chartData.entries)
      ...
      children: [
        Row(
          ...
          children: [
            ...
            const Spacer(),
            Obx(() => Text(controller.isSwitched.value ? 'Chart' : 'Value',
                style: Get.textTheme.labelMedium!.copyWith(fontSize: isUnfolded ? 14 : (isSmallScreen ? 14 : 14)))),
            Obx(() => Transform.scale(
                  scale: 0.6,
        ...
        Obx(() => Text(
              controller.showAlternateAggregateData.value
                  ? 'Berdasarkan Jumlah Keberangkatan Penumpang'
                  : 'Berdasarkan Jumlah Kedatangan Penumpang',
  ...
  Widget _buildAggregateChart(BuildContext context) {
    return Obx(() {
      ...
      return SizedBox(
        height: 300,
        child: _buildAggregateCartesianChart(context, chartData, maxValue),
      );
    });
  }
        ...
        return Container(
          ...
          child: Obx(() {
            ...
            return DataTable(
              ...
              rows: _buildTableRows(list, context),
            );
          }),
        );
      },
  ...
  List<DataColumn> _buildTableColumns(BuildContext context) {
    ...
    return [
      ...
      DataColumn(
        label: Text(
          'Simpul',
          style: TextStyle(fontWeight: FontWeight.bold, fontSize: isUnfolded ? 13 : (isSmallScreen ? 10 : 11)),
        ),
      ),
      DataColumn(
        numeric: true,
        label: SizedBox(
          ...
          child: Padding(
            padding: EdgeInsets.only(left: isUnfolded ? 60.0 : (isSmallScreen ? 10.0 : 15.0)),
            child: SvgPicture.asset(
              AssetConstant.users,
              height: 15,
            ),
      ...
      DataColumn(
        ...
        label: SizedBox(
          ...
          child: Padding(
            padding: EdgeInsets.only(left: isUnfolded ? 33.0 : (isSmallScreen ? 5.0 : 10.0)),
            child: SvgPicture.asset(
              AssetConstant.shipIcon,
              height: 17,
            ),
  ...
  List<DataRow> _buildTableRows(List<AggregateChartData> list, BuildContext context) {
    final screenHeight = MediaQuery.of(context).size.height;
    final screenWidth = MediaQuery.of(context).size.width;
    final isSmallScreen = screenHeight < 700 || screenWidth < 380;
    final isUnfolded = screenWidth > 600;
  ...
  Widget _buildAggregateCartesianChart(
      BuildContext context, List<AggregateChartData> chartData, double maxValue) {
    final screenHeight = MediaQuery.of(context).size.height;
    final screenWidth = MediaQuery.of(context).size.width;
    final isSmallScreen = screenHeight < 700 || screenWidth < 380;
    final isUnfolded = screenWidth > 600;
    return SfCartesianChart(
      ...
      series: <CartesianSeries<AggregateChartData, String>>[
        StackedBarSeries<AggregateChartData, String>(
          ...
          xValueMapper: (data, _) =>
              data.name != '' ? data.name : data.idLocation,
