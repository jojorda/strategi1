...
class LautController extends GetxController with GetTickerProviderStateMixin {
  final ModaRepository _modaRepository;
  final StrategiRepository _strategiRepository;

  LautController(this._modaRepository, this._strategiRepository);
  ...
  void onInit() {
    ...
    ever(selectedOdDateRange, (_) => fetchData(true));
  ...
  List<LautEnum> getMenuList() {
    return [
      if (hasPermission(Permission.lautAgregat)) LautEnum.agregat,
      LautEnum.wilayah,
      LautEnum.simpul,
      if (hasPermission(Permission.lautOD)) LautEnum.od,
      if (hasPermission(Permission.lautOperator)) LautEnum.operator,
      if (hasPermission(Permission.lautKecelakaan)) LautEnum.kecelakaan,
      LautEnum.kejadianMenonjol,
    ];
  }

  int getSelectedIndex(LautEnum lautEnum) {
    final list = getMenuList();
    final index = list.indexOf(lautEnum);
    return index != -1 ? index : 0;
  }

  initIndex() {
    if (menuArgument.value?.lautEnum != null) {
      selectedIndex.value = getSelectedIndex(menuArgument.value!.lautEnum!);
    } else {
  ...
  Future<Event?> getEvent() async {
    ...
    _strategiRepository.getEvent(EventType.laut).listen((events) {
