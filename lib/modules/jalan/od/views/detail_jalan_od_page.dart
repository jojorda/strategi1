...
class DetailJalanOdPage extends GetView<JalanOdDetailController> {
  static String routeName = '/detail-jalan-od-page';
  const DetailJalanOdPage({super.key});
                    ...
                    children: [
                      Text(
                        ...
                        style: Get.textTheme.bodyMedium!
                            .copyWith(fontWeight: FontWeight.w600, fontSize: isUnfolded ? 14 : (isSmallScreen ? 14 : FontSizes.s12)),
                      ),
                      SizedBox(height: Sizes.s4),
                      Text(
                        "${controller.originType.isEmpty ? '' : 'TT${controller.originType}  | '}${controller.originCityName.isEmpty ? '-' : controller.originCityName}",
                        style: Get.textTheme.labelMedium!.copyWith(
                            fontWeight: FontWeight.w400, fontSize: isUnfolded ? 14 : (isSmallScreen ? 14 : FontSizes.s10)),
                      ),
                      SizedBox(height: Sizes.s10),
                      _buildMobilitySection(OdEnum.origin),
                      ...
                      SizedBox(height: Sizes.s4),
                      Text(
                        "${controller.destinationType.isEmpty ? '' : 'TT${controller.destinationType}  | '}${controller.destinationCityName.isEmpty ? '-' : controller.destinationCityName}",
                        style: Get.textTheme.labelMedium!.copyWith(
                            fontWeight: FontWeight.w400, fontSize: isUnfolded ? 14 : (isSmallScreen ? 14 : FontSizes.s10)),
                      ),
                      SizedBox(height: isUnfolded ? Sizes.s6 : (isSmallScreen ? Sizes.s10 : Sizes.s10)),
                      _buildMobilitySection(OdEnum.destination),
                    ],
                  ),
                  ),
                ),
  ...
  Widget _buildMobilitySection(OdEnum odEnum) {
    return Obx(() {
      return Stack(
        alignment: Alignment.center,
        children: [
          _buildGraphCard(odEnum),
          Visibility(
            visible: !controller.isLoadingOdDetailData.value &&
  ...
  Widget _buildGraphCard(OdEnum odEnum) {
    return Obx(() {
  ...
  Widget _buildGraphContent(OdEnum odEnum) {
    return Padding(
  ...
  Widget _buildGraphLegend(OdEnum odEnum) {
    return Obx(() {
  ...
  Widget _buildLegendItem({
    ...
    return Row(
      children: [
        ...
        Text(
          value,
          style: Get.textTheme.labelMedium!.copyWith(color: color, fontSize: isUnfolded ? 14 : (isSmallScreen ? 12 : 14)),
        ),
        const SizedBox(width: 2),
        Text(label, style: Get.textTheme.labelMedium!.copyWith(fontSize: isUnfolded ? 14 : (isSmallScreen ? 12 : 14)),),
      ],
    );
  }
    ...
    return Obx(() {
      ...
      if (trafficData == null) {
        return SizedBox(
          height: 200,
          child: Center(
            child: Text('Memuat data', style: Get.textTheme.labelLarge!.copyWith(fontSize: isUnfolded ? 14 : (isSmallScreen ? 14 : FontSizes.s12))),
          ),
        );
      }
  ...
  double _calculateChartMaxValue(TrafficData trafficData, OdEnum odEnum) {
    if (odEnum == OdEnum.origin && controller.showVehicleDataOrigin.value ||
      ...
      final arrivals = (trafficData.arrival).map((e) => e.value).fold(1.0, max);
      return max(departures, arrivals);
    }
  }
