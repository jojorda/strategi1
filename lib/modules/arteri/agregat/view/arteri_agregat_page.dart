...
import 'package:strategi_hub_app/components/BarChart/BarChartArteri.dart';

class ArteriAgregat extends GetView<ArteriController> {
  static const routeName = '/aggregate';

  const ArteriAgregat({super.key});
  ...
  Widget _buildLegendItem({
    ...
    return Row(
      children: [
        ...
        Text(
          value,
          style: Get.textTheme.labelMedium!.copyWith(color: color,fontSize: isUnfolded ? 12 : (isSmallScreen ? 10 : 12)),
        ),
        const SizedBox(width: 2),
        Text(label, style: Get.textTheme.labelMedium!.copyWith(fontSize: isUnfolded ? 12 : (isSmallScreen ? 10 : 12))),
      ],
    );
  }
  ...
  Widget _buildChartBar() {
    return Obx(() {
      ...
      final chartColor = controller.showAlternateAggregateData.value ? AppColors.chartColor : Colors.pink;
    ...
    return <CartesianSeries<ChartData, String>>[
      LineSeries<ChartData, String>(
        ...
        dataSource: controller.showVehicleData.value
            ? trafficData.vehicleDeparture?.where((data) => data.value != 0).toList()
            : trafficData.departure?.where((data) => data.value != 0).toList(),
      ...
      LineSeries<ChartData, String>(
        name: "Tiba",
        dataSource: controller.showVehicleData.value
            ? trafficData.vehicleArrival?.where((data) => data.value != 0).toList()
            : trafficData.arrival?.where((data) => data.value != 0).toList(),
        xValueMapper: (ChartData data, _) {
          // Filter harian - tampilkan format tanggal
          if (controller.isRoutine.value && controller.selectedFilter.value == 0) {
            // Dapatkan tanggal dari data
      ...
      children: [
        ...
        Obx(() => Text(
              controller.showAlternateAggregateData.value
                  ? 'Berdasarkan Jumlah Kendaraan Keluar Jabodetabek'
                  : 'Berdasarkan Jumlah Kendaraan Masuk Jabodetabek',
              style: TextStyle(fontSize: isUnfolded ? 13 : (isSmallScreen ? 10 : 12)),
            )),
      ],
    );
  }
  ...
  Widget _buildAggregateChart() {
    return Obx(() {
      final chartData = controller.currentAggregateData.value?.chart?.where((data) =>
          (controller.showAlternateAggregateData.value
              ? data.vehicleDeparture
              : data.vehicleArrival) != 0).toList();
  ...
  List<AggregateChartData>? _getSortedAggregateData() {
    ...
    final sorted = List<AggregateChartData>.from(list)
      ...
      series: <CartesianSeries<AggregateChartData, String>>[
        StackedBarSeries<AggregateChartData, String>(
          ...
          yValueMapper: (data, _) => controller.showAlternateAggregateData.value
              ? data.vehicleDeparture
              : data.vehicleArrival,
          name: controller.showAlternateAggregateData.value ? 'Keluar' : 'Masuk',
