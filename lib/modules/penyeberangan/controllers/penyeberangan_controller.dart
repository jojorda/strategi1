...
// Penyebrangan's Moda Type is ASDP
class PenyeberanganController extends GetxController
    with GetTickerProviderStateMixin {
  final ModaRepository _modaRepository;
  final StrategiRepository _strategiRepository;
  PenyeberanganController(this._modaRepository, this._strategiRepository);
  ...
  Rx<od.OdChartList?> seasonalListOdData = Rx<od.OdChartList?>(null);
  ...
  void onInit() {
    ...
    debounce(searchOperator, (_) {
      getOperatorsData(isRefresh: true, search: searchOperator.value);
  ...
  List<PenyebranganEnum> getMenuList() {
    return [
      if (hasPermission(Permission.penyebranganAgregat))
        PenyebranganEnum.agregat,
      PenyebranganEnum.wilayah,
      PenyebranganEnum.simpul,
      if (hasPermission(Permission.penyebranganOD)) PenyebranganEnum.od,
      if (hasPermission(Permission.penyebranganOperator))
        PenyebranganEnum.operator,
      if (hasPermission(Permission.penyebranganKecelakaan))
        PenyebranganEnum.kecelakaan,
      PenyebranganEnum.kejadianMenonjol,
    ];
  }

  int getSelectedIndex(PenyebranganEnum penyebranganEnum) {
    final list = getMenuList();
    final index = list.indexOf(penyebranganEnum);
    return index != -1 ? index : 0;
  }

  void initIndex() {
    if (menuArgument.value?.penyebranganEnum != null) {
      selectedIndex.value =
          getSelectedIndex(menuArgument.value!.penyebranganEnum!);
    } else {
  ...
  void getOdData({bool isRefresh = false, String search = ''}) async {
    ...
    if (!isRoutine.value && seasonalListOdData.value != null && !isRefresh) {
      currentListOdData.value = seasonalListOdData.value;
      return;
    }
        ...
        (result) {
          if (result != null) {
            ...
            if (isRoutine.value) {
              ...
              seasonalListOdData.value = result;
              currentListOdData.value = seasonalListOdData.value;
            }
  ...
  void applySort() {
    if (sortBy.value == currentSortBy.value) return;
